...
//    }
    ...
    fun handleChildClass(ourClass: KtClass, ourFqName: String, holder: ProblemsHolder) {
        ...
        val toLookFor = inputParameterNames.toMap(
            keyMapper = { it },
            valueMapper = { QuickNameLookupValue(0, false) }
        )
        ...
        functionsOverriding.forEach { function ->
            ...
            if (haveCachedFnc != null && haveCachedFnc.second == function.modificationStamp) {
                ...
                if (usesConstructorParameterInOverridden.isNotEmpty() && superProblemsNames.contains(
                        function.name
                            ?: ""
                    )
                ) {
                    val refNames =
                        usesConstructorParameterInOverridden.map { exp -> exp.getReferencedName() }.distinct()
    ...
    fun computeBaseClassDangerousStarts(
        ...
        val namesToLookForSet = namesToLookFor.toMap(
            keyMapper = { it },
            valueMapper = { QuickNameLookupValue(0, false) }
        )
